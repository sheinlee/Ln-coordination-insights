...
with open(element+'_phenathroline2.txt','a') as f:
    f.close()

for h in hits:
    print(h.identifier)
    with open(element+'_phenanthroline2.txt','a') as f:
        f.write(h.identifier)
        f.write("\n")

# 去除重复项
fi = open(element+'_phenanthroline2.txt','r')
txt = fi.readlines()
with open(element+'_phenanthroline2_OK.txt','a') as f:
    f.close()
for w in txt:
    fi2 = open(element+'_phenanthroline2_OK.txt','r')
    txt2 = fi2.readlines()
    with open(element+'_phenanthroline2_OK.txt','a') as f:
        if w not in txt2:
            f.write(w)
