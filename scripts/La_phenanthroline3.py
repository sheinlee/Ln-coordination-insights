...
with open(element+'_phenanthroline3.txt','a') as f:
    f.close()
...
# 去除重复项
fi = open(element+'_phenanthroline3.txt','r')
txt = fi.readlines()
with open(element+'_phenanthroline3_OK.txt','a') as f:
    f.close()
for w in txt:
    fi2 = open(element+'_phenanthroline3_OK.txt','r')
    txt2 = fi2.readlines()
    with open(element+'_phenanthroline3_OK.txt','a') as f:
        if w not in txt2:
            f.write(w)
